...
class DoublyLinkedList:
    ...
    def add_to_head(self, value):
        ...
        self.length += 1
        # what if there is no head and no tail?
        if not self.head and not self.tail:
            # set the only node to both the head and the tail
            self.head = new_node
            self.tail = new_node
        else:
            new_node.next = self.head #new node is now the head
    ...
    def delete(self, node):
        ...
        if self.head is self.tail:
            ...
            node.delete()
        # if node is neither head nor tail, delete it without resetting head or tail
        node.delete()
...
# from Sean Chen's lecture on doubly-linked-lists, lecture code from notepad channel
